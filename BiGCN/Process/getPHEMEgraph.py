...
def main():
    ...
    labelPath = os.path.join(cwd, "data/PHEME/PHEME_id_label.txt")
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez(os.path.join(cwd,'data/PHEMEgraph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
        x_word, x_index, tree, rootfeat, rootindex = constructMat(event)
        x_x = getfeature(x_word, x_index)
