...
class BertStancePooler(nn.Module):
    ...
    def forward(self, hidden_states, max_tweet_num, max_tweet_len):
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        # max_tweet_len = 34  # the number of words in each tweet (30, 17) performs best
...
class BertForSeqStanceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids1, token_type_ids1, attention_mask1, input_ids2, token_type_ids2, attention_mask2,
                ...
                input_ids7, token_type_ids7, attention_mask7, input_ids8, token_type_ids8, attention_mask8,
                input_ids9, token_type_ids9, attention_mask9, input_ids10, token_type_ids10, attention_mask10,
        ...
        sequence_output7, _ = self.bert(input_ids7, token_type_ids7, attention_mask7, output_all_encoded_layers=False)
        sequence_output8, _ = self.bert(input_ids8, token_type_ids8, attention_mask8, output_all_encoded_layers=False)
        sequence_output9, _ = self.bert(input_ids9, token_type_ids9, attention_mask9, output_all_encoded_layers=False)
        sequence_output10, _ = self.bert(input_ids10, token_type_ids10, attention_mask10, output_all_encoded_layers=False)

        tmp_sequence = torch.cat((sequence_output1, sequence_output2), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output3), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output4), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output5), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output6), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output7), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output8), dim=1)
        tmp_sequence = torch.cat((tmp_sequence, sequence_output9), dim=1)
        sequence_output = torch.cat((tmp_sequence, sequence_output10), dim=1)
...
class FullySharedBert(PreTrainedBertModel):
    def __init__(self, config, rumor_num_labels=2, stance_num_labels=2):
    ...
    def forward(self, input_ids1, token_type_ids1, attention_mask1, input_ids2, token_type_ids2, attention_mask2,
        ...
        if task is None: #for rumor detection task
            ...
            # print(stance_logits.shape) #### [batch_size, 7:each example seven prediction, 6+1 class: main+pad class]
...
class DualBert(PreTrainedBertModel):
    ...
    def forward(self, input_ids1, token_type_ids1, attention_mask1, input_ids2, token_type_ids2, attention_mask2,
        ...
        if task is None:  # for rumor detection task
            ...
            stance_label_layer = self.add_self_attention(stance_logits, extended_label_mask)
