...
class RumorDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        Output: 将一个thread视作一个batch
            ...
            - edge_index_TD: [[int]. [int]], shape: [2, |E|], 自顶向下的传播树图, E是边集
            - edge_index_BU: [[int]. [int]], shape: [2, |E|], 自底向上的传播树图, E是边集
            - root_index: int
...
class RumorStanceDataset(Dataset):
    def __init__(self, data_path: str, type: str, tokenizer: BertTokenizer):
        ...
        self.post_id2label = {}
        for id, label in zip(self.all_post_id, self.all_post_label):
            self.post_id2label[id] = label
    ...
    def __getitem__(self, item):
        ...
        for id in ids:
            ...
            if id in self.all_post_id:
                thread.append([id, self.all_post[id]['text']])
                stance_label.append(int(self.post_id2label[id]))
        root_index, node_text, edge_index_TD = self.structure2graph(thread_id, thread, structure)
    ...
    def collate_fn(batch):
        ...
        Output: 将一个thread视作一个batch
            ...
            - edge_index_BU: [[int]. [int]], shape: [2, |E|], 自底向上的传播树图, E是边集
            - root_index: int
            - rumor_label: Tensor, shape = [1]
            - stance_label: Tensor, shape = [|N|], N是点集
        '''
        for item in batch:
            root_index = item[1]
            node_text = item[2]
        ...
        return node_token, edge_index_TD, edge_index_BU, root_index,\
               torch.LongTensor([int(rumor_label)]), torch.LongTensor(stance_label)